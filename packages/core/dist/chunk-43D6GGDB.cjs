...
function bootstrapMiddleware(config = {}) {
  return async (c, next) => {
    ...
    try {
      ...
      const migrationService = new chunkQVRPPS6Z_cjs.MigrationService(c.env.DB);
      await migrationService.runPendingMigrations();
      console.log("[Bootstrap] Syncing collection configurations...");
      try {
        await chunk3ADEYHN2_cjs.syncCollections(c.env.DB);
      } catch (error) {
        console.error("[Bootstrap] Error syncing collections:", error);
      }
      console.log("[Bootstrap] Syncing form collections...");
      try {
        await chunk3ADEYHN2_cjs.syncAllFormCollections(c.env.DB);
      } catch (error) {
        console.error("[Bootstrap] Error syncing form collections:", error);
      }
      if (!config.plugins?.disableAll) {
        console.log("[Bootstrap] Bootstrapping core plugins...");
        const bootstrapService = new chunk3ADEYHN2_cjs.PluginBootstrapService(c.env.DB);
...
var AuthManager = class _AuthManager {
   ...
   */
  static async ensureCredentialAccount(db, userId, passwordHash) {
    const now = Date.now();
    await db.prepare(
...
var requireRbac = (resource, verb) => {
  return async (c, next) => {
    ...
    const allowed = await new chunkINCD27QJ_cjs.RbacService(c.env.DB).can(user.userId, resource, verb);
    if (!allowed) {
      const acceptHeader = c.req.header("Accept") || "";
      if (acceptHeader.includes("text/html")) {
        return c.redirect("/auth/login?error=You do not have permission to access this area");
...
//# sourceMappingURL=chunk-43D6GGDB.cjs.map
//# sourceMappingURL=chunk-43D6GGDB.cjs.map