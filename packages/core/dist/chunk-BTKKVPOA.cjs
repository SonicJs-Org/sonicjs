...
function bootstrapMiddleware(config = {}) {
  return async (c, next) => {
    ...
    try {
      ...
      const migrationService = new chunk5VAL7Z3S_cjs.MigrationService(c.env.DB);
...
var requireRbac = (resource, verb) => {
  return async (c, next) => {
    ...
    if (cachedPerms !== void 0) {
      ...
      allowed = await new chunk323GE63K_cjs.RbacService(c.env.DB).can(user.userId, resource, verb);
    }
    if (!allowed) {
      const acceptHeader = c.req.header("Accept") || "";
      if (acceptHeader.includes("text/html")) {
...
//# sourceMappingURL=chunk-BTKKVPOA.cjs.map
//# sourceMappingURL=chunk-BTKKVPOA.cjs.map