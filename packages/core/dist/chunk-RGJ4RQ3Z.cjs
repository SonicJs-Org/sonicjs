...
var Logger = class {
  ...
  async log(level, category, message, data, context) {
    ...
    try {
      ...
      await this.db.insert(chunk4RVMY6FI_cjs.systemLogs).values(logEntry);
      if (config.maxSize) {
  ...
  async getLogs(filter = {}) {
    try {
      ...
      if (filter.level && filter.level.length > 0) {
        conditions.push(drizzleOrm.inArray(chunk4RVMY6FI_cjs.systemLogs.level, filter.level));
      }
      if (filter.category && filter.category.length > 0) {
        conditions.push(drizzleOrm.inArray(chunk4RVMY6FI_cjs.systemLogs.category, filter.category));
      }
      if (filter.userId) {
        conditions.push(drizzleOrm.eq(chunk4RVMY6FI_cjs.systemLogs.userId, filter.userId));
      }
      if (filter.source) {
        conditions.push(drizzleOrm.eq(chunk4RVMY6FI_cjs.systemLogs.source, filter.source));
      }
      if (filter.search) {
        conditions.push(
          drizzleOrm.like(chunk4RVMY6FI_cjs.systemLogs.message, `%${filter.search}%`)
        );
      }
      if (filter.startDate) {
        conditions.push(drizzleOrm.gte(chunk4RVMY6FI_cjs.systemLogs.createdAt, filter.startDate));
      }
      if (filter.endDate) {
        conditions.push(drizzleOrm.lte(chunk4RVMY6FI_cjs.systemLogs.createdAt, filter.endDate));
      }
      const whereClause = conditions.length > 0 ? drizzleOrm.and(...conditions) : void 0;
      const totalResult = await this.db.select({ count: drizzleOrm.count() }).from(chunk4RVMY6FI_cjs.systemLogs).where(whereClause);
      const total = totalResult[0]?.count || 0;
      const sortColumn = filter.sortBy === "level" ? chunk4RVMY6FI_cjs.systemLogs.level : filter.sortBy === "category" ? chunk4RVMY6FI_cjs.systemLogs.category : chunk4RVMY6FI_cjs.systemLogs.createdAt;
      const sortFn = filter.sortOrder === "asc" ? drizzleOrm.asc : drizzleOrm.desc;
      const logs = await this.db.select().from(chunk4RVMY6FI_cjs.systemLogs).where(whereClause).orderBy(sortFn(sortColumn)).limit(filter.limit || 50).offset(filter.offset || 0);
      return { logs, total };
    } catch (error) {
  ...
  async getConfig(category) {
    try {
      ...
      const configs = await this.db.select().from(chunk4RVMY6FI_cjs.logConfig).where(drizzleOrm.eq(chunk4RVMY6FI_cjs.logConfig.category, category));
  ...
  async updateConfig(category, updates) {
    try {
      await this.db.update(chunk4RVMY6FI_cjs.logConfig).set({
        ...updates,
        updatedAt: /* @__PURE__ */ new Date()
      }).where(drizzleOrm.eq(chunk4RVMY6FI_cjs.logConfig.category, category));
  ...
  async getAllConfigs() {
    try {
      return await this.db.select().from(chunk4RVMY6FI_cjs.logConfig);
    } catch (error) {
  ...
  async cleanupCategory(category, maxSize) {
    try {
      const countResult = await this.db.select({ count: drizzleOrm.count() }).from(chunk4RVMY6FI_cjs.systemLogs).where(drizzleOrm.eq(chunk4RVMY6FI_cjs.systemLogs.category, category));
      const currentCount = countResult[0]?.count || 0;
      if (currentCount > maxSize) {
        const cutoffLogs = await this.db.select({ createdAt: chunk4RVMY6FI_cjs.systemLogs.createdAt }).from(chunk4RVMY6FI_cjs.systemLogs).where(drizzleOrm.eq(chunk4RVMY6FI_cjs.systemLogs.category, category)).orderBy(drizzleOrm.desc(chunk4RVMY6FI_cjs.systemLogs.createdAt)).limit(1).offset(maxSize - 1);
        if (cutoffLogs[0]) {
          await this.db.delete(chunk4RVMY6FI_cjs.systemLogs).where(
            drizzleOrm.and(
              drizzleOrm.eq(chunk4RVMY6FI_cjs.systemLogs.category, category),
              drizzleOrm.lte(chunk4RVMY6FI_cjs.systemLogs.createdAt, cutoffLogs[0].createdAt)
            )
          );
        }
      }
    ...
    try {
      ...
      for (const config of configs) {
        if (config.retention > 0) {
          ...
          await this.db.delete(chunk4RVMY6FI_cjs.systemLogs).where(
            drizzleOrm.and(
              drizzleOrm.eq(chunk4RVMY6FI_cjs.systemLogs.category, config.category),
              drizzleOrm.lte(chunk4RVMY6FI_cjs.systemLogs.createdAt, cutoffDate)
            )
          );
        }
      }
...
//# sourceMappingURL=chunk-RGJ4RQ3Z.cjs.map
//# sourceMappingURL=chunk-RGJ4RQ3Z.cjs.map