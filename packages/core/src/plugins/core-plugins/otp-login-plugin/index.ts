      ...
      try {
        ...
        if (emailPlugin?.settings) {
          ...
          if (
            ...
            if (!emailResponse.ok) {
              const errorData = (await emailResponse.json()) as {
                message?: string;
              };
  ...
  otpAPI.post("/verify", async (c: any) => {
    try {
      ...
      if (!user && settings.allowNewUserRegistration) {
        ...
        const now = Date.now();
        const username =
          normalizedEmail.split("@")[0] + "_" + userId.slice(0, 6);
        ...
        `,
          ...
          .bind(userId, normalizedEmail, username, now, now)
          .run();
