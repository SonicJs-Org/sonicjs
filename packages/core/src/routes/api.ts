...
// executionCtx is absent in test environments (Hono throws if accessed on a plain Request).
// Guard every scheduleKvWrite call through this helper so tests don't 500.
function tryCtx(c: { executionCtx: unknown }): { waitUntil(p: Promise<unknown>): void } | null {
  try { return c.executionCtx as { waitUntil(p: Promise<unknown>): void } } catch { return null }
}
  ...
  try {
    ...
    if (cacheEnabled) {
      ...
      if (cacheResult.hit && cacheResult.data) {
        ...
        scheduleKvWrite(cacheKey, tryCtx(c))
        return c.json(dataWithMeta)
      }
