...
describe('MigrationService', () => {
  describe('consolidated greenfield migrations', () => {
    it('exposes only the greenfield core migrations', async () => {
      ...
      expect(migrations.map(m => m.id)).toEqual(['0001', '0002', '0003', '0004', '0005'])
    ...
    it('should read applied migrations from d1_migrations', async () => {
      const db = createMockDb({
        appliedMigrations: [
          ...
          { name: '0005_documents_fts.sql', applied_at: '2026-01-01T00:00:04.000Z' },
        ],
        existingTables: ['users', 'documents', 'document_types'],
        existingColumns: []
      })
      ...
      expect(status.appliedMigrations).toBe(5)
      expect(status.pendingMigrations).toBe(0)
      expect(status.lastApplied).toBe('2026-01-01T00:00:04.000Z')
    })
  })
})
