...
- 7-day default JWT TTL (`JWT_SECRET` env var required)
...
JWTs are verified on every request by `requireAuth()` in `packages/core/src/middleware/auth.ts`. SonicJS signs and verifies tokens with `hono/jwt` using the `JWT_SECRET` env var. No external session store is needed for JWT verification.

Better Auth sessions (for the admin UI) are stored in Cloudflare KV (`CACHE_KV`) as secondary storage so `getSession()` skips a D1 read on hot paths — but this does not apply to the JWT Bearer path used by the API.
...
```typescript
// src/index.ts
import { createSonicJSApp, registerCollections, registerPlugins, getHookSystem } from '@sonicjs-cms/core'
import {
  authPlugin,
...
const app = createSonicJSApp({
  plugins: { register: [], disableAll: false }
})
