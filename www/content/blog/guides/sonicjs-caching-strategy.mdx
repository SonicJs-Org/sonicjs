...
```typescript
// TODO: verify hook event name 'content:afterSave' for v3
definePlugin({
  onBoot(ctx) {
    ctx.hooks.on('content:afterSave', async (content) => {
...
```typescript
// TODO: verify hook event name 'content:beforeCache' for v3
definePlugin({
  onBoot(ctx) {
    ctx.hooks.on('content:beforeCache', (content, options) => {
