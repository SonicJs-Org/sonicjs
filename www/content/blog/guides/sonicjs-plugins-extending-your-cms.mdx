...
A plugin in SonicJS is a plain TypeScript object that conforms to the `Plugin` interface. There is no compiled DSL, no YAML, no separate runtime. You build a plugin and register it with `registerPlugins(app, [myPlugin], { hookSystem, env })` after calling `createSonicJSApp(config)`, and it gets wired into the same Hono app that serves the rest of the CMS.
