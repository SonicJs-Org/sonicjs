...
The admin API reference page auto-discovers endpoints at runtime, so it always reflects the currently available routes including any registered plugins.
...
Most API endpoints require authentication. SonicJS uses Better Auth for session management. Programmatic API clients (scripts, external apps) authenticate with a JWT/API-token Bearer header; browser and admin UI sessions use a Better Auth session cookie set automatically on login.
...
Retrieve all content items for a collection with pagination and filtering.
...
- `sort` - JSON array of sort objects: `[{"field":"created_at","order":"desc"}]`
- `filter` - Advanced filtering (see [Filtering & Querying](#filtering))

<CodeGroup title="List Content">

```bash {{ title: 'cURL' }}
curl -X GET "http://localhost:8787/api/blog-posts?limit=10"
```

```js {{ title: 'JavaScript' }}
const response = await fetch('http://localhost:8787/api/blog-posts?limit=10');
const { data, meta } = await response.json();
```
...
response = requests.get('http://localhost:8787/api/blog-posts', params={
    'limit': 10
})
data = response.json()
```
...
```bash {{ title: 'cURL' }}
curl -X GET "http://localhost:8787/api/blog-posts/{id}"
```

```js {{ title: 'JavaScript' }}
const response = await fetch(`http://localhost:8787/api/blog-posts/${id}`);
...
Create a new content item. The request body is the field data for the collection directly — no wrapper object needed.

**Authentication:** Required
**Permissions:** Admin, Editor, or Author role

<CodeGroup title="Create Content">

```bash {{ title: 'cURL' }}
curl -X POST "http://localhost:8787/api/blog-posts" \
  -H "Authorization: Bearer {token}" \
  -H "Content-Type: application/json" \
  -d '{
...
```js {{ title: 'JavaScript' }}
const response = await fetch('http://localhost:8787/api/blog-posts', {
  method: 'POST',
  headers: {
    'Authorization': `Bearer ${token}`,
    'Content-Type': 'application/json',
...
```bash {{ title: 'cURL' }}
curl -X PUT "http://localhost:8787/api/blog-posts/{id}" \
  -H "Authorization: Bearer {token}" \
  -H "Content-Type: application/json" \
  -d '{
...
```bash {{ title: 'cURL' }}
curl -X DELETE "http://localhost:8787/api/blog-posts/{id}" \
  -H "Authorization: Bearer {token}"
```
...
Pass `sort` as a URL-encoded JSON array of `{field, order}` objects. `order` is `"asc"` or `"desc"`.

```bash
# Sort by created_at descending
curl "http://localhost:8787/api/content?sort=%5B%7B%22field%22%3A%22created_at%22%2C%22order%22%3A%22desc%22%7D%5D"

# Sort by title ascending
curl "http://localhost:8787/api/content?sort=%5B%7B%22field%22%3A%22title%22%2C%22order%22%3A%22asc%22%7D%5D"
```

```js
// JavaScript — construct and encode the sort param
const sort = JSON.stringify([{ field: 'created_at', order: 'desc' }])
const url = `/api/blog-posts?sort=${encodeURIComponent(sort)}`
```
