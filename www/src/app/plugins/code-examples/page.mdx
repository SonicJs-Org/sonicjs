...
```typescript
import { codeExamplesPlugin } from '@sonicjs-cms/core'
```
...
const codeExamplesPlugin = definePlugin({
  // ... other fields ...
  install: async (ctx) => {
    await ctx.db.prepare(codeExampleMigration).run()
  },
  uninstall: async (ctx) => {
    await ctx.db.prepare('DROP TABLE IF EXISTS code_examples').run()
  },
  activate: async (ctx) => { /* logs activation */ },
  deactivate: async (ctx) => { /* logs deactivation */ },
})
```
