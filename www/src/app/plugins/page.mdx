...
export const sections = [
  ...
  { title: 'Authentication (Better Auth)', id: 'authentication-better-auth' },
...
- **Authentication** - Magic link and email OTP via Better Auth config
...
**Required for:**
- Better Auth magic link / email OTP (when configured)
...
Passwordless login (magic link, email OTP) is added via **Better Auth plugins** in your app config, not as admin-activated plugins.

- **Magic link** — Add the `magicLink` plugin in `SonicJSConfig.auth.extendBetterAuth` and implement `sendMagicLink`. See [Authentication - Magic Link](/authentication#magic-link--email-otp).
- **Email OTP** — Add the `emailOtp` plugin in `auth.extendBetterAuth` and implement `sendVerificationOTP`. See [Authentication - Email OTP](/authentication#magic-link--email-otp).

Configure your Email Plugin so Better Auth can send magic link and OTP emails when those methods are enabled.
