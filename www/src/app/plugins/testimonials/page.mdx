...
```typescript
import { testimonialsPlugin } from '@sonicjs-cms/core'
```
...
const testimonialsPlugin = definePlugin({
  // ... other fields ...
  install: async (ctx) => {
    await ctx.db.prepare(testimonialMigration).run()
  },
  uninstall: async (ctx) => {
    await ctx.db.prepare('DROP TABLE IF EXISTS testimonials').run()
  },
