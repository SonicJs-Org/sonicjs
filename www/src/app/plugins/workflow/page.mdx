...
<div className="p-4 mb-6 rounded-lg border border-amber-300 dark:border-amber-700 bg-amber-50 dark:bg-amber-900/20">
  <strong>⚠️ Experimental / Inactive in v3</strong> — Workflow routes are currently disabled. Draft/publish lifecycle is handled natively by the Document Model (<code>is_current_draft</code> / <code>is_published</code> flags). This page documents the plugin architecture for reference.
</div>
...
{/* Note: workflow plugin is experimental in v3 — API subject to change */}
<CodeGroup title="Perform Workflow Action">

```typescript
import { WorkflowManager, WorkflowAction, ContentStatus } from '@sonicjs-cms/core'
...
```typescript
import { WorkflowManager, WorkflowPermissions } from '@sonicjs-cms/core'
...
```typescript
import { ContentWorkflow, WorkflowAction, ContentStatus } from '@sonicjs-cms/core'
...
```typescript
import { WorkflowEngine } from '@sonicjs-cms/core'
...
```typescript
import { WorkflowManager } from '@sonicjs-cms/core'
...
const workflowPlugin = definePlugin({
  name: 'workflow',
  onBoot(ctx) {
    ctx.hooks.on('content:create', async (data, context) => {
      if (context?.db) {
...
import { WorkflowManager, WorkflowAction } from '@sonicjs-cms/core'
...
```typescript
import { ContentWorkflow, ContentStatus } from '@sonicjs-cms/core'
