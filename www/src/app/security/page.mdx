  ...
  features={[
    {
      ...
      description: 'Better Auth session-based auth with HTTP-only cookies',
    },
    {
      icon: '🛡️',
...
SonicJS uses **Better Auth** for sign-in and sessions with these security features:

- **Session cookie** — HTTP-only `better-auth.session_token` (no client-side token handling)
- **Configurable expiration** — Session lifetime and refresh via Better Auth options
- **Secure storage** — Sessions stored in database and/or cookie by Better Auth

### Cookie Security

Better Auth sets the session cookie with secure defaults (httpOnly, sameSite, secure in production). Cookie options are configurable in your Better Auth config.

### Password Security

Better Auth hashes and verifies passwords for sign-in and sign-up. Do not use custom password hashing for new features.

<Note>
Always set a strong **BETTER_AUTH_SECRET** (min 32 characters) in production. See [Deployment](/deployment).
</Note>

### Multi-Factor Authentication

For 2FA or passwordless login, add Better Auth plugins in `auth.extendBetterAuth`:

<CodeGroup title="Better Auth plugins">

```typescript
// In SonicJSConfig.auth.extendBetterAuth:
// 1. Add magicLink or emailOtp plugin from better-auth/plugins
// 2. Implement sendMagicLink or sendVerificationOTP (e.g. with Email Plugin)
// See Authentication docs for full examples.
```
