...
**Causes:**
- Session cookie not sent (e.g. missing `credentials: 'include'` or wrong origin)
- Session expired or invalid
- `BETTER_AUTH_SECRET` or `BETTER_AUTH_URL` mismatch

**Solutions:**

<CodeGroup title="Include session cookie">

```typescript
// Browser: send credentials so session cookie is included
fetch('/api/content', {
  credentials: 'include'
})

// Ensure BETTER_AUTH_URL matches the app URL (same origin for cookies)
```
...
**Causes:**
- Session has expired (lifetime configured in Better Auth)
- User signed out elsewhere

**Solutions:**

<CodeGroup title="Re-authenticate">

```typescript
// Session refresh is handled by Better Auth; if session is gone, sign in again
// Redirect to login
