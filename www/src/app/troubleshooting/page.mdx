...
// Admin user is created via bootstrap using ADMIN_EMAIL/ADMIN_PASSWORD env vars
// Demo credentials: admin@sonicjs.com / sonicjs!
// Additional users are created through the admin UI or Better Auth registration flow
```
...
```typescript
import { CacheInvalidationService } from '@sonicjs-cms/core'
...
```typescript
import { PluginValidator } from '@sonicjs-cms/core'
...
// Register in your app entry point (my-sonicjs-app/src/index.ts)
...
// Collections are code-only via registerCollections() — no DB sync needed
// Ensure registerCollections([...]) is called in your app entry point
```
