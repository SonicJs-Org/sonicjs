...
const TIERS = ['Priority Support', 'Enterprise', 'Not sure yet'] as const
const SIZES = [
  'Pre-production / evaluating',
  '1–10 sites',
  '11–50 sites',
  '50+ sites',
] as const
...
export function SupportContactForm() {
  ...
  // Load Turnstile script once, then render the widget into our ref.
  useEffect(() => {
    function render() {
  ...
  async function onSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      const res = await fetch('/api/commercial-support', {
        method: 'POST',
  ...
  if (status === 'success') {
    return (
      <div className="rounded-2xl bg-emerald-50/50 p-6 text-sm text-emerald-900 ring-1 ring-emerald-500/20 ring-inset dark:bg-emerald-500/5 dark:text-emerald-200 dark:ring-emerald-500/30">
        <p className="font-semibold">Thanks — we&rsquo;ve got your request.</p>
        <p className="mt-1">
          A member of the team will reply within 1 business day.
        </p>
      </div>
    )
  }
    ...
    <form onSubmit={onSubmit} className="not-prose grid gap-4">
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        <Field label="Tier of interest">
          <select name="tier" required className={inputClasses}>
            {TIERS.map((t) => (
      ...
      <Field label="How can we help? (optional — procurement/security forms, timelines, etc.)">
      ...
      <div>
        ...
        >
          {status === 'submitting' ? 'Sending…' : 'Request support'}
        </button>
      </div>
    </form>
  )
